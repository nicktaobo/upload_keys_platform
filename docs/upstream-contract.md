...
- 上游用户名和密码由管理员在 KeyHub 的 Upstream 页面录入，使用应用加密密钥加密后存入 PostgreSQL。浏览器只调用 KeyHub API，不直接访问上游。
- 第一版只使用一个共享上游账号，并自动选择名称同时包含 `Claude` 和 `官方` 的渠道。下游用户不能选择渠道。
...
不要把凭据、Cookie 或 Token 放入 `.env.example`。
...
`id` 可为字符串或数字，进入 KeyHub 后统一转为字符串。`name` 必须为字符串。Worker 当前选择第一个名称同时包含 `Claude` 和 `官方` 的渠道，并把其 ID 保存到主上游连接记录。
...
虽然适配器支持数组，当前提交 Worker 每个任务只发送一行。完整 Key 只在 Worker 解密后用于该请求，不应进入日志、错误消息或任务 payload。
...
{
  "success": true,
  "itemIds": ["<upstream-item-id>"]
}
```

也接受蛇形字段 `item_ids`；其中每个 ID 可为字符串或数字并统一转为字符串。`itemIds`/`item_ids` 缺失时会映射为空数组。Worker 只有在 `success === true` 且数组首项存在时才将本地记录标记为 `SUBMITTED` 并保存 `upstreamItemId`；否则标记为 `TEST_FAILED`，错误信息为通用脱敏文本。
...
| `status` | 无 | `status` | 可选字符串 |
| `usage_usd` | `usageUsd` | `usageUsd` | 可选数字 |
| `usage_site_count` | `usageSiteCount` | `usageSiteCount` | 可选非负整数 |
| `sampled_at` | `sampledAt` | `sampledAt` | 可选字符串；Worker 只保存可解析日期 |
| `next_cursor` | `next` | `nextCursor` | 可选字符串或 `null` |

裸数组视为没有下一页。若分页对象同时提供 `next_cursor` 和 `next`，当前优先使用 `next_cursor`。同步最多读取 100 页，之后不再继续。同步只按已保存的 `upstreamItemId` 匹配本地记录；未映射的上游记录会被忽略，按用户手动同步时还会额外限制 `ownerId`。
...
- 上游原始错误响应及可能含敏感信息的响应字段；
- 带 query/body/header 的完整请求转储。

当前适配器错误类型只暴露通用文本、错误分类和可选 HTTP 状态码，并有测试保证超时及 HTTP 错误不会串入凭据、Cookie、CSRF 或 Key-like 值。Worker 持久化的失败信息也是固定脱敏文本。API 当前关闭 Fastify logger；未来启用结构化日志时，必须先对上述字段名和 Key-like 值配置递归脱敏，并添加回归测试。
