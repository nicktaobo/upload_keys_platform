import { Button, ConfigProvider, Drawer, Layout, Menu, Tooltip, Typography } from "antd";
...
function Shell({ user, onLogout }: { user: SessionUser; onLogout: () => void }) {
  ...
  const userItems = [{ key: "/keys", icon: <Database size={17} />, label: <Link to="/keys">My Keys</Link> }, { key: "/submit", icon: <Send size={17} />, label: <Link to="/submit">Submit Key</Link> }];
  const adminItems = [{ key: "/admin/users", icon: <Users size={17} />, label: <Link to="/admin/users">Users</Link> }, { key: "/admin/keys", icon: <Database size={17} />, label: <Link to="/admin/keys">All Keys</Link> }, { key: "/admin/upstream", icon: <Server size={17} />, label: <Link to="/admin/upstream">Upstream</Link> }, { key: "/admin/operations", icon: <Activity size={17} />, label: <Link to="/admin/operations">Operations</Link> }];
...
export function App() { return <ConfigProvider theme={theme}><BrowserRouter><AppRoutes /></BrowserRouter></ConfigProvider>; }
