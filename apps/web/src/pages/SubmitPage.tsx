...
export function SubmitPage() {
  ...
  return <>{context}<Typography.Title level={2}>Submit Key</Typography.Title><Typography.Paragraph type="secondary">Keys are submitted to the fixed Claude official API channel.</Typography.Paragraph><div className="channel-strip"><span>Channel</span><strong>Claude official API</strong></div>
    ...
    <Tabs items={[
      ...
      { key: "batch", label: "Batch paste", children: <Form form={batchForm} layout="vertical" className="form-column" onFinish={({ rows }) => void handle(() => api.submitBatch(rows), () => batchForm.resetFields(["rows"]))}><Form.Item label="Keys and warranty hours" name="rows" extra="One Key per row. Warranty hours are optional and default to 1." rules={[{ required: true, message: "Paste at least one row" }]}><Input.TextArea className="batch-textarea" rows={9} wrap="off" spellCheck={false} placeholder={"sk-ant-...\nsk-ant-..."} /></Form.Item><div className="batch-stats" aria-label="Batch statistics"><Typography.Text type="secondary">Total rows <strong>{batchStats.totalRows}</strong></Typography.Text><Typography.Text type="success">Ready to submit <strong>{batchStats.submitableRows}</strong></Typography.Text><Typography.Text type="warning">Duplicates <strong>{batchStats.duplicateRows}</strong></Typography.Text><Typography.Text type="danger">Invalid <strong>{batchStats.invalidRows}</strong></Typography.Text></div><Button type="primary" htmlType="submit" icon={<Send size={16} />} loading={busy}>Submit batch</Button></Form> },
    ]} />
  </>;
}
