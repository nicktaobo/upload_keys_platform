...
export function KeysPage() {
  ...
  const load = useCallback(async () => {
    ...
    try { const [nextSummary, list] = await Promise.all([api.summary(), api.keys(status)]); setSummary(nextSummary); setRecords(list.items); }
  ...
  const reveal = async (record: KeyRecord) => { try { const result = await api.reveal(record.id); await navigator.clipboard.writeText(result.key); setRevealed(result.key); } catch (cause) { toast.error(cause instanceof Error ? cause.message : "Reveal failed"); } };
  return <>{toastContext}<Flex justify="space-between" align="start" gap={16} wrap><div><Typography.Title level={2}>My Keys</Typography.Title><Typography.Paragraph type="secondary">Submission status and current upstream usage.</Typography.Paragraph></div><Space><Button aria-label="Refresh data" icon={<RefreshCw size={16} />} loading={refreshing} onClick={() => void refresh()}>Refresh</Button><Link to="/submit"><Button type="primary" icon={<Send size={16} />}>Submit Key</Button></Link></Space></Flex>
    ...
    <div className="table-toolbar"><Select aria-label="Filter by status" placeholder="All statuses" allowClear value={status} onChange={setStatus} options={["pending", "submitting", "submitted", "test_failed", "retrying", "upstream_error"].map((value) => ({ value, label: value.replaceAll("_", " ").replace(/^./, (x) => x.toUpperCase()) }))} /></div>
    <AsyncState loading={loading} error={error} empty={!loading && !error && records.length === 0}><KeyTable records={records} onReveal={(record) => void reveal(record)} /></AsyncState>
