import { Button, Flex, Modal, Select, Space, Statistic, Typography, message } from "antd";
...
export function KeysPage() {
  ...
  const [retry, setRetry] = useState<KeyRecord | null>(null), [retrying, setRetrying] = useState(false);
  ...
  const confirmRetry = async () => {
    if (!retry) return;
    setRetrying(true);
    try { toast.success((await api.retryOwnedKey(retry.id)).message); setRetry(null); await load(); }
  ...
  return <>{toastContext}<Flex justify="space-between" align="start" gap={16} wrap><div><Typography.Title level={2}>{t("keys.title", "My Keys")}</Typography.Title><Typography.Paragraph type="secondary">{t("keys.subtitle", "Submission status and current upstream usage.")}</Typography.Paragraph></div><Space><Button aria-label="Refresh data" icon={<RefreshCw size={16} />} loading={refreshing} onClick={() => void refresh()}>{t("action.refresh", "Refresh")}</Button><Link to="/submit"><Button type="primary" icon={<Send size={16} />}>{t("submit.submit", "Submit Key")}</Button></Link></Space></Flex>
    {summary && <div className="metrics"><Statistic title={t("keys.submitted", "Submitted Keys")} value={summary.total} /><Statistic title={t("keys.healthy", "Healthy")} value={summary.healthy} /><Statistic title={t("keys.usage", "Accumulated usage")} value={summary.usageUsd} formatter={() => `$${summary.usageUsd.toFixed(2)}`} /><Statistic title={t("keys.latest", "Latest sample")} value={summary.latestSampleAt ? formatDateTime(summary.latestSampleAt) : t("common.noSamples", "No samples")} /></div>}
    <div className="table-toolbar"><Select aria-label="Filter by status" placeholder={t("filter.allStatuses", "All statuses")} allowClear value={status} onChange={(nextStatus) => { setStatus(nextStatus); setPage(1); }} options={["pending", "submitting", "submitted", "test_failed", "retrying", "upstream_error"].map((value) => ({ value, label: t(`status.${value}`, value.replaceAll("_", " ").replace(/^./, (x) => x.toUpperCase())) }))} /></div>
    ...
    <Modal title="Retry submission" open={retry !== null} onCancel={() => setRetry(null)} onOk={() => void confirmRetry()} okText="Retry" confirmLoading={retrying}>Queue another submission attempt for <strong>{retry?.maskedKey}</strong>?</Modal>
