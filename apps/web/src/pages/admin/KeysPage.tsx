import { Card, Flex, Modal, Select, Statistic, Typography, message } from "antd";
...
export function AdminKeysPage() {
  ...
  const selectedOwner = ownerId ? owners.find((owner) => owner.id === ownerId)?.username : undefined;
  return <>{context}<Typography.Title level={2}>{t("adminKeys.title", "All Keys")}</Typography.Title><Typography.Paragraph type="secondary">{t("adminKeys.subtitle", "Masked records across all owners, including sanitized failure details.")}</Typography.Paragraph>
    <Flex gap={12} wrap="wrap" className="table-toolbar"><Select aria-label="Filter by account" allowClear showSearch optionFilterProp="label" placeholder={t("adminKeys.allAccounts", "All accounts")} value={ownerId} onChange={(value) => { setOwnerId(value); setPage(1); }} options={owners.map((owner) => ({ value: owner.id, label: owner.username }))} /><Select aria-label="Filter by status" allowClear placeholder={t("filter.allStatuses", "All statuses")} value={status} onChange={(value) => { setStatus(value); setPage(1); }} options={["pending", "submitting", "submitted", "test_failed", "retrying", "upstream_error"].map((value) => ({ value, label: t(`status.${value}`, value.replaceAll("_", " ").replace(/^./, (letter) => letter.toUpperCase())) }))} /></Flex>
    <Flex gap={12} wrap="wrap" className="admin-total-metrics"><Card size="small"><Statistic title={t("adminKeys.users", "Users")} value={selectedOwner ?? stats.length} /></Card><Card size="small"><Statistic title={t("adminKeys.keys", "Keys")} value={aggregate.keyCount} /></Card><Card size="small"><Statistic title={t("adminKeys.healthy", "Healthy")} value={aggregate.healthyCount} /></Card><Card size="small"><Statistic title={t("adminKeys.usage", "Usage")} value={aggregate.usageUsd} precision={2} prefix="$" /></Card></Flex>
