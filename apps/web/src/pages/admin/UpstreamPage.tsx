...
export function UpstreamPage() {
  ...
  return <>{context}<div className="page-heading"><div><Typography.Title level={2}>{t("upstream.title", "Upstream")}</Typography.Title><Typography.Paragraph type="secondary">{t("upstream.subtitle", "Shared supplier connection and synchronization controls.")}</Typography.Paragraph></div><Button type="primary" icon={<RefreshCw size={16} />} loading={syncing} onClick={() => void sync()}>{t("action.sync", "Sync now")}</Button></div><AsyncState loading={loading} error={error}>{connection && <><>{connection.state === "blocked" && <Alert className="blocking-alert" type="error" showIcon message={t("upstream.blocked", "Automation is blocked")} description={connection.failureMessage} />}</><Descriptions size="small" bordered column={{ xs: 1, sm: 3 }} items={[{ key: "state", label: t("upstream.state", "State"), children: connection.state }, { key: "login", label: t("upstream.lastLogin", "Last login"), children: format(connection.lastLoginAt) }, { key: "sync", label: t("upstream.lastSync", "Last sync"), children: format(connection.lastSyncAt) }]} /><Typography.Title level={4}>{t("upstream.credentials", "Credentials")}</Typography.Title><Form layout="vertical" className="form-column" initialValues={{ username: connection.username }} onFinish={save}><Form.Item label={t("upstream.username", "Upstream username")} name="username" rules={[{ required: true }]}><Input autoComplete="off" /></Form.Item><Form.Item label={t("upstream.password", "Upstream password")} name="password" rules={[{ required: true }]}><Input.Password autoComplete="new-password" /></Form.Item><Space><Button type="primary" htmlType="submit" icon={<Save size={16} />} loading={saving}>{t("upstream.save", "Save credentials")}</Button></Space></Form></>}</AsyncState></>;
}
